...
git show > $1/git-show
git diff > $1/git-diff
cp examples/imagenet/train_googlenet16.sh $1/.
...
cp examples/imagenet/16parts/ps_config_googlenet $1/.
mpirun -machinefile examples/imagenet/16parts/machinefile ./build/tools/caffe_mpi train --solver=examples/imagenet/16parts/googlenet_solver.prototxt --ps_config=examples/imagenet/16parts/ps_config_googlenet 2>&1 | tee $1/output.txt
